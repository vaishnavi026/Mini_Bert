...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        ...
        # initial size of query, val and keys are : [bs, num_attention_heads, seq_len, attention_head_size]
        d_k = query.shape[-1]
        attention_scores = (query @ key.transpose(-2, -1)) / math.sqrt(d_k) # [bs, num_attention_heads, seq_len, seq_len]
        # replace masked values with very negative scores
        attention_scores = attention_scores + attention_mask
        # normalize the scores
        attention_scores = attention_scores.softmax(dim=-1)
        # multiply the attention scores to the value and get back V'
        value = attention_scores @ value # [bs, num_attention_heads, seq_len, head_size]
        # next, we need to concat multi-heads and recover the original shape [bs, seq_len, num_attention_heads * attention_head_size = hidden_size]
        # [bs, num_attention_heads, seq_len, head_size ] -->
        # [bs, seq_len, num_attention_heads, head_size]  -->
        # [bs, seq_len, num_attention_heads * attention_head_size]
        value = value.transpose(1, 2).contiguous().view(value.shape[0], -1, self.all_head_size)
        return value
...
class BertLayer(nn.Module):
    ...
    This function computes ``LayerNorm(input + Sublayer(output))``, where sublayer is a dense_layer
    followed by dropout.
    """
    # todo
        op = dropout(dense_layer(output))
        return ln_layer(input + op)
    ...
    """
        ...
        # add-norm layer
        norm_out = self.add_norm(hidden_states, att_out, self.attention_dense, self.attention_dropout,
                                 self.attention_layer_norm)

        # feed forward
        ff_out = self.interm_af(self.interm_dense(norm_out))

        # another add-norm layer
        norm_out = self.add_norm(norm_out, ff_out, self.out_dense, self.out_dropout, self.out_layer_norm)
